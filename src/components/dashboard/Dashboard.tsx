...
export default function Dashboard({ user }: DashboardProps) {
  ...
  const slides = [
    ...
    {
      title: "Tuntematon ystävä",
      content: <TasteTwinCard userEmail={userEmail} />
    },
    {
      title: "Vastaantulijoiden kaistalla",
  ...
  return (
      ...
      {/* User Selector - Bottom (only for tommi.forsstrom@gmail.com) */}
      {user.email === 'tommi.forsstrom@gmail.com' && (
        <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-40">
          <select 
            value={userEmail}
            onChange={(e) => setUserEmail(e.target.value)}
            className="bg-black/80 text-white text-xs px-2 py-1 rounded border border-white/20 max-w-xs"
          >
            ...
            <option value="imattila@welho.com">imattila@welho.com</option>
